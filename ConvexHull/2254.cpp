...
int main()
{
    ...
    while (looper && newv.size() >= 3)
    {
        ...
        int initret = ccw(cvhull[0], cvhull[1], og);
        checker.insert(cvhull[0]);
        bool nope = 0;
        for (int i = 1; i < cvhull.size(); i++)
        {
            checker.insert(cvhull[i]);
            if (ccw(cvhull[i], cvhull[(i + 1) % cvhull.size()], og) != initret)
            {
