...
int main()
{
    ...
    cin >> N;
    vector<int> v(N+2), H(N+2), L(N+2);
    for(int i = 1; i <= N; i++)cin>>v[i];
    for(int i = 1; i <= N; i++)cin>>H[i];
    for(int i = 1; i <= N; i++)cin>>L[i];
    int maxnode = max_element(v.begin(),v.end())-v.begin();

    int source = 602, sink = 603;
    MCMF<ll> mf(605, source, sink);

    for(int i = N; i; i--){
        mf.add_edge(source,i<<1,(i==maxnode?L[i]:L[i]-1),0);
    ...
    for (int i = 1; i < N; i++)
    {
        for(int j = i+1; j <= N; j++){
            int a=i,b=j;
            if(v[b]>v[a])swap(a,b);
            mf.add_edge(a<<1,b<<1|1,1,H[a]+H[b]-(v[a]^v[b]));
        }
    }
    pair<ll,ll> p = mf.run();
    cout << -p.first;
