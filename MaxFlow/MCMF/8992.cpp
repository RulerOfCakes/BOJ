...
int main()
{
    ...
    while(T--){
        cin >> N>>M;
        vector<line> gv,sv;
        int source = 812, sink = 813;
        MCMF<int> mf(815, source, sink);
        for(int i = 0; i < N; i++){
            int a,b,c,d,w;
            cin>>a>>b>>c>>d>>w;
            if(a>c)swap(a,c);
            gv.push_back({a,b,c,d,w});
            mf.add_edge(i<<1,i<<1|1,1,0);
        }
        //int icount=0;
        for(int i = 0; i < M; i++){
            int a,b,c,d,w;
            cin>>a>>b>>c>>d>>w;
            if(b<d)swap(b,d);
            sv.push_back({a,b,c,d,w});
            int ri=i+N;
            mf.add_edge(ri<<1,ri<<1|1,1,0);
            for(int i= 0; i < N; i++){
                if(gv[i].a<a&&gv[i].c>a&&b>gv[i].b&&d<gv[i].d){
                    //intersection
                    mf.add_edge(source,i<<1,1,0);
                    mf.add_edge(i<<1|1,ri<<1,1,-gv[i].w*w);
                    mf.add_edge(ri<<1|1,sink,1,0);
                }
            }
        }
